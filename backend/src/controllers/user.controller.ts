...
export async function register(req: Request, res: Response) {
  ...
  try {
    const out = await registerUser(email.trim().toLowerCase(), password);
    return res.status(201).json(out);
  } catch (e: any) {
    if (e.message === "email_exists") return res.status(409).json({ error: "Email já cadastrado" });
...
export async function login(req: Request, res: Response) {
  ...
  try {
    const out = await loginUser(email.trim().toLowerCase(), password);
    return res.json(out); 
  } catch (e: any) {
